...
cstr = {0: "blue", 1: "red", 2: "green", 3: "yellow", 4: "cyan", 5: "magenta", 6: "brown", 7: "violet"}

if "-l" in sys.argv:
	for p in range(k):
		print "\\begin{tabular}{|cccccccc|} \hline"
		for i in range(n):
			for j in range(n):
				print "\cellcolor{" + cstr[A[p][i][j]] + "!25}" + str(A[p][i][j]) + (" \\\\" if j==n-1 else " &"),
			print ("" if i != n-1 else "\hline")
		print "\end{tabular}" + ("\n\quad" if p != k-1 else "")
