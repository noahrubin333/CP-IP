...
class MyCallback: public GRBCallback
{
    void callback()
    {
        ...
        if(where==GRB_CB_MIP)
        {
            ...
            cout << "Node " << nodenum << " Cuts " << cuts << endl;
        }
#endif
        if(where==GRB_CB_MIPNODE && getIntInfo(GRB_CB_MIPNODE_STATUS) == GRB_OPTIMAL)
        {
            GRBVar xvar[10][10][10][10];
            for(int i=0; i<N; i++)
                for(int j=0; j<N; j++)
                    for(int k=0; k<N; k++)
                        for(int l=0; l<N; l++)
                            xvar[i][j][k][l] = Gx[i*N*N*N+j*N*N+k*N+l];
#ifdef CLIQUETYPEII
                                    ...
                                    if(xt > (1-xc)/(double)3)
                                    {
                                        ...
                                        for(int s2=0; s2<ysize; s2++)
                                        {
                                            ...
                                            if(INT1(i,j,k,l,ipp,jpp,kpp,lpp))
                                            {
                                                ...
                                                {   LOGCUTIII; addCut(xvar[i][j][k][l]+xvar[i][j][kp][lp]+xvar[i][jp][k][lp]+xvar[i][jp][kp][l], GRB_LESS_EQUAL, 1.0); return; }
                                                if(j==jpp && xc+getNodeRel(xvar[i][j][kp][lp])+getNodeRel(xvar[ip][j][k][lp])+getNodeRel(xvar[ip][j][kp][l])>1)
                                                {   LOGCUTIII; addCut(xvar[i][j][k][l]+xvar[i][j][kp][lp]+xvar[ip][j][k][lp]+xvar[ip][j][kp][l], GRB_LESS_EQUAL, 1.0); return; }
                                                if(k==kpp && xc+getNodeRel(xvar[i][jp][k][lp])+getNodeRel(xvar[ip][j][k][lp])+getNodeRel(xvar[ip][jp][k][l])>1)
                                                {   LOGCUTIII; addCut(xvar[i][j][k][l]+xvar[i][jp][k][lp]+xvar[ip][j][k][lp]+xvar[ip][jp][k][l], GRB_LESS_EQUAL, 1.0); return; }
                                                if(l==lpp && xc+getNodeRel(xvar[i][jp][kp][l])+getNodeRel(xvar[ip][j][kp][l])+getNodeRel(xvar[ip][jp][k][l])>1)
                                                {   LOGCUTIII; addCut(xvar[i][j][k][l]+xvar[i][jp][kp][l]+xvar[ip][j][kp][l]+xvar[ip][jp][k][l], GRB_LESS_EQUAL, 1.0); return; }
                                            }
                                        }
                                    }
                                }
